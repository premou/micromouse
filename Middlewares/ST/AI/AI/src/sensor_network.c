...
#define AI_NET_OBJ_INSTANCE g_sensor_network
 
#undef AI_SENSOR_NETWORK_MODEL_SIGNATURE
#define AI_SENSOR_NETWORK_MODEL_SIGNATURE     "d761f01ac1b5ff327466bcd204d11502"
...
#undef AI_SENSOR_NETWORK_N_BATCHES
#define AI_SENSOR_NETWORK_N_BATCHES         (1)
...
AI_TENSOR_OBJ_DECLARE(
  dense_1_weights,
  AI_SHAPE_INIT(1, 1, 16, 4),
  AI_STRIDE_INIT(256, 256, 16, 4),
...
AI_TENSOR_OBJ_DECLARE(
  dense_2_weights,
  AI_SHAPE_INIT(1, 1, 8, 16),
  AI_STRIDE_INIT(512, 512, 64, 4),
...
AI_DECLARE_STATIC
ai_bool sensor_network_configure_activations(
...
AI_DECLARE_STATIC
ai_bool sensor_network_configure_weights(
  ai_network* net_ctx, const ai_buffer* weights_buffer)
{
  ...
  dense_1_bias_array.data = weights + 256;
  dense_1_bias_array.data_start = weights + 256;
  dense_2_weights_array.format |= AI_FMT_FLAG_CONST;
  dense_2_weights_array.data = weights + 320;
  dense_2_weights_array.data_start = weights + 320;
  dense_2_bias_array.format |= AI_FMT_FLAG_CONST;
  dense_2_bias_array.data = weights + 832;
  dense_2_bias_array.data_start = weights + 832;
...
AI_API_ENTRY
ai_bool ai_sensor_network_get_info(
  ai_handle network, ai_network_report* report)
{
  ...
  {
    ai_network_report r = {
      .model_name        = AI_SENSOR_NETWORK_MODEL_NAME,
      .model_signature   = AI_SENSOR_NETWORK_MODEL_SIGNATURE,
      ...
      .n_macc            = 288,
      .n_inputs          = AI_SENSOR_NETWORK_IN_NUM,
      .inputs            = AI_BUFFER_OBJ_INIT(
                              ...
                              4,
                              1, NULL),
      .n_outputs         = AI_SENSOR_NETWORK_OUT_NUM,
...
AI_API_ENTRY
ai_error ai_sensor_network_get_error(ai_handle network)
{
  return ai_platform_network_get_error(network);
}

AI_API_ENTRY
ai_error ai_sensor_network_create(
...
AI_API_ENTRY
ai_handle ai_sensor_network_destroy(ai_handle network)
{
  return ai_platform_network_destroy(network);
}

AI_API_ENTRY
ai_bool ai_sensor_network_init(
  ai_handle network, const ai_network_params* params)
{
  ...
  ai_bool ok = true;
  ok &= sensor_network_configure_weights(net_ctx, &params->params);
  ok &= sensor_network_configure_activations(net_ctx, &params->activations);
...
AI_API_ENTRY
ai_i32 ai_sensor_network_forward(ai_handle network, const ai_buffer* input)
{
