...
% with Bernard, 1/sqrt(ADC) is almost LINEAR
% y = 1/(ax+b)^2
% x = (1/sqrt(y)-b)/a
close all
clear
...
load data.txt
% extract raw data
distance = data([2:140],1);
raw = [ data([2:140],2), data([2:140],3), data([2:140],4), data([2:140],5) ];
% median filter on raw data
filt_raw = medfilt2(raw,[5 1]);
%figure(1);
figure(1,"position",[0,0,1920,1040])
subplot(2,3,1)
plot(filt_raw)
legend("DL","FL","FR","DR")
...
%figure(5);
subplot(2,3,5)
distance_reference = [2:1:180];
plot(synthetic_distance)
hold on;
plot(distance_reference,":k")
legend("DL","FL","FR","DR", "REF")
...
%figure(6);
subplot(2,3,6)
error_reference = zeros(179,1);
%plot([error,error_reference])
plot(error);
hold on;
plot(error_reference,":k")
legend("DL","FL","FR","DR","REF")
